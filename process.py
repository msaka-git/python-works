...
def writing_logs():
    with open("/tmp/monitoring_logs." + ts_local +".csv", "a") as file:
...
def log_header():
    head=[""]
    with open("/tmp/monitoring_logs." + ts_local +".csv", "w") as file:
        file.write("Date;Hostname;Applications;Status;Commentaires\n")

os.remove("/tmp/monitoring_logs."+host)
log_header()
for i in ("firefox", "bash"):
    check_kill_process(i)
    writing_logs()

os.symlink("/tmp/monitoring_logs." + ts_local +".csv", "/tmp/monitoring_logs."+host)
